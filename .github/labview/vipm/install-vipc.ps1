...
$PublicRepoUrl    = if ($Env:VIPM_PUBLIC_REPO_URL) { $Env:VIPM_PUBLIC_REPO_URL } else { 'https://github.com/MIpetrov-NI/measurement-plugin-python.git' }
...
function Save-PublicVipmPackage($Package, [string] $OutDir) {
    ...
    Invoke-WebRequest -Uri $url -OutFile $outFile -UseBasicParsing -MaximumRedirection 10 -TimeoutSec 300 -Headers @{ 'User-Agent' = 'measurement-plugin-python VIPM downloader' } | Out-Null
